...
def predict_sentiment(sentence, tokenizer, model):
    ...
    SEQ_LEN = 150 # 최대 token 개수 이상의 값으로 임의로 설정
    ...
    predicted_class = ['fear','surprise', 'anger','sadness','happiness','disgust'][np.argmax(prediction, axis=1)[0]]# ex) ['부정', '긍정'][[1][0]] -> ['부정', '긍정'][1] -> '긍정'
